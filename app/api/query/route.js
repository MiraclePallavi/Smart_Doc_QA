...
// Initialize the Gemini API client safely with an options configuration object
...
export async function POST(req) {
  try {
    ...
    // 📥 Read dictionary database files from disk
    ...
    // 🧠 Step 1: Classify user intent to route to the correct chunk size dictionary key
    ...
    // Clean up Gemini's text response to eliminate trailing whitespaces, newlines, or markdown wrapper tags
    let targetTask = intentResponse.text.trim().toUpperCase().replace(/[^A-Z]/g, "");
    
    // Strict safety fallback optimization
    if (targetTask !== "QA" && targetTask !== "SUMMARY") {
      targetTask = "QA"; 
    }

    // 🎯 Step 2: Extract the exact task array matching your dictionary format
    const chunks = allChunks[targetTask];
    const embeddings = allEmbeddings[targetTask];

    // Double check safety validation before mapping vectors
    if (!chunks || !embeddings) {
      return Response.json({ error: `Requested task data for '${targetTask}' was not found.` }, { status: 500 });
    }

    // 🤖 Step 3: Get query embedding via your local Python script
    ...
    // 🔍 Step 4: Vector similarity search mapping across the isolated arrays safely
    ...
    // Filter relevant context segments
    ...
    // 🧠 Step 5: GENERATION phase powered by Gemini
